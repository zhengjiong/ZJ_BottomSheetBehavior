...
public class CustomBottomSheetBehavior2<V extends View> extends CoordinatorLayout.Behavior<V> {
     ...
     */
    public CustomBottomSheetBehavior2(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private final ViewDragHelper.Callback mDragCallback = new ViewDragHelper.Callback() {
        ...
        public boolean tryCaptureView(View child, int pointerId) {
            ...
            if (mState == STATE_EXPANDED && mActivePointerId == pointerId) {
                View scroll1 = mNestedScrollingChildRef1.get();
                View scroll2 = mNestedScrollingChildRef2.get();
                if (scroll1 != null && ViewCompat.canScrollVertically(scroll1, -1)) {
                    // Let the content scroll up
                    return false;
                }
                if (scroll2 != null && ViewCompat.canScrollVertically(scroll2, -1)) {
                    // Let the content scroll up
                    return false;
                }
            }
    ...
    private static class SavedState extends AbsSavedState {
        ...
        private SavedState(Parcelable superState, @State int state) {
            super(superState);
            this.state = state;
        }
        ...
        public static final Creator<SavedState> CREATOR = ParcelableCompat.newCreator(
                new ParcelableCompatCreatorCallbacks<CustomBottomSheetBehavior2.SavedState>() {
                    @Override
                    public CustomBottomSheetBehavior2.SavedState createFromParcel(Parcel in, ClassLoader loader) {
                        return new CustomBottomSheetBehavior2.SavedState(in, loader);
                    }

                    @Override
                    public CustomBottomSheetBehavior2.SavedState[] newArray(int size) {
                        return new CustomBottomSheetBehavior2.SavedState[size];
                    }
                });
    }

    /**
     * A utility function to get the {@link CustomBottomSheetBehavior2} associated with the {@code view}.
     *
     * @param view The {@link View} with {@link CustomBottomSheetBehavior2}.
     * @return The {@link CustomBottomSheetBehavior2} associated with the {@code view}.
     */
    @SuppressWarnings("unchecked")
    public static <V extends View> CustomBottomSheetBehavior2<V> from(V view) {
        ViewGroup.LayoutParams params = view.getLayoutParams();
        if (!(params instanceof CoordinatorLayout.LayoutParams)) {
            throw new IllegalArgumentException("The view is not a child of CoordinatorLayout");
        }
        CoordinatorLayout.Behavior behavior = ((CoordinatorLayout.LayoutParams) params).getBehavior();
        if (!(behavior instanceof CustomBottomSheetBehavior2)) {
