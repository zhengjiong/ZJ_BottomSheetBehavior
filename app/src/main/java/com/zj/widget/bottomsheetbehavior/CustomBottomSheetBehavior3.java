...
/**
 * CreateTime: 17/11/30 15:33
 *
 * @author zhengjiong
 */
public class CustomBottomSheetBehavior3<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    public abstract static class BottomSheetCallback {
         ...
         */
        public abstract void onStateChanged(@NonNull View bottomSheet, @CustomBottomSheetBehavior3.State int newState);
    ...
    @CustomBottomSheetBehavior3.State
    ...
    WeakReference<View> mNestedScrollingChildRef1;
    WeakReference<View> mNestedScrollingChildRef2;

    private CustomBottomSheetBehavior3.BottomSheetCallback mCallback;
    ...
    boolean mTouchingScrollingChild1;
    boolean mTouchingScrollingChild2;

    /**
     * Default constructor for instantiating BottomSheetBehaviors.
     */
    public CustomBottomSheetBehavior3() {
    }
     ...
     */
    public CustomBottomSheetBehavior3(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public Parcelable onSaveInstanceState(CoordinatorLayout parent, V child) {
        return new CustomBottomSheetBehavior3.SavedState(super.onSaveInstanceState(parent, child), mState);
    }

    @Override
    public void onRestoreInstanceState(CoordinatorLayout parent, V child, Parcelable state) {
        CustomBottomSheetBehavior3.SavedState ss = (CustomBottomSheetBehavior3.SavedState) state;
    ...
    public boolean onInterceptTouchEvent(CoordinatorLayout parent, V child, MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL:
                mTouchingScrollingChild1 = false;
                mTouchingScrollingChild2 = false;
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                View scroll1 = mNestedScrollingChildRef1 != null
                        ? mNestedScrollingChildRef1.get() : null;
                View scroll2 = mNestedScrollingChildRef2 != null
                        ? mNestedScrollingChildRef2.get() : null;
                if (scroll1 != null && parent.isPointInChildBounds(scroll1, initialX, mInitialY)) {
                    mActivePointerId = event.getPointerId(event.getActionIndex());
                    mTouchingScrollingChild1 = true;
                }
                if (scroll2 != null && parent.isPointInChildBounds(scroll2, initialX, mInitialY)) {
                    mActivePointerId = event.getPointerId(event.getActionIndex());
                    mTouchingScrollingChild2 = true;
                }
        ...
        boolean result1 = action == MotionEvent.ACTION_MOVE && (scroll1 != null || scroll2 != null) &&
        ...
        //boolean allResult = result1 && result2;
    ...
    public void onStopNestedScroll(CoordinatorLayout coordinatorLayout, V child, View target) {
        ...
        /*if (mNestedScrollingChildRef1 == null || target != mNestedScrollingChildRef1.get()
                || !mNestedScrolled) {
            return;
        }*/
        if ((mNestedScrollingChildRef1 == null && mNestedScrollingChildRef2 == null) ||
                (target != mNestedScrollingChildRef1.get() && target != mNestedScrollingChildRef2.get()) ||
                !mNestedScrolled) {
            return;
        }
        ...
        if (mViewDragHelper.smoothSlideViewTo(child, child.getLeft(), top)) {
            ...
            ViewCompat.postOnAnimation(child, new CustomBottomSheetBehavior3.SettleRunnable(child, targetState));
        } else {
     ...
     */
    public void setBottomSheetCallback(CustomBottomSheetBehavior3.BottomSheetCallback callback) {
        mCallback = callback;
    }
     ...
     *              {@link #STATE_HIDDEN}.
     */
    public final void setState(final @CustomBottomSheetBehavior3.State int state) {
        if (state == mState) {
            return;
        }
        if (mViewRef == null) {
    ...
    void setStateInternal(@CustomBottomSheetBehavior3.State int state) {
        if (mState == state) {
            return;
        }
        mState = state;
        ...
        if (view instanceof ViewGroup) {
            ...
            for (int i = 0, count = group.getChildCount(); i < count; i++) {
                ...
                if (scrollingChild != null) {
                    if (mNestedScrollingChildRef1 != null && mNestedScrollingChildRef1.get() == scrollingChild) {
                        continue;
                    }
    ...
    void startSettlingAnimation(View child, int state) {
        ...
        if (mViewDragHelper.smoothSlideViewTo(child, child.getLeft(), top)) {
            setStateInternal(STATE_SETTLING);
            ViewCompat.postOnAnimation(child, new CustomBottomSheetBehavior3.SettleRunnable(child, state));
        } else {
    ...
    private final ViewDragHelper.Callback mDragCallback = new ViewDragHelper.Callback() {
        ...
        public boolean tryCaptureView(View child, int pointerId) {
            ...
            if (mState == STATE_EXPANDED && mActivePointerId == pointerId) {
                View scroll1 = mNestedScrollingChildRef1.get();
                View scroll2 = mNestedScrollingChildRef2.get();
                /*if (scroll1 != null && scroll1.canScrollVertically(-1)) {
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            int top;
            @CustomBottomSheetBehavior3.State int targetState;
            if (yvel < 0) { // Moving up
                top = mMinOffset;
                targetState = STATE_EXPANDED;
            ...
            if (mViewDragHelper.settleCapturedViewAt(releasedChild.getLeft(), top)) {
                ...
                ViewCompat.postOnAnimation(releasedChild,
                        new CustomBottomSheetBehavior3.SettleRunnable(releasedChild, targetState));
            } else {
                setStateInternal(targetState);
            }
        }
    ...
    private class SettleRunnable implements Runnable {
        ...
        SettleRunnable(View view, @CustomBottomSheetBehavior3.State int targetState) {
            mView = view;
    ...
    protected static class SavedState extends AbsSavedState {
        ...
        public SavedState(Parcelable superState, @CustomBottomSheetBehavior3.State int state) {
            super(superState);
            this.state = state;
        }
        ...
        public static final Creator<CustomBottomSheetBehavior3.SavedState> CREATOR = new ClassLoaderCreator<CustomBottomSheetBehavior3.SavedState>() {
            @Override
            public CustomBottomSheetBehavior3.SavedState createFromParcel(Parcel in, ClassLoader loader) {
                return new CustomBottomSheetBehavior3.SavedState(in, loader);
            }

            @Override
            public CustomBottomSheetBehavior3.SavedState createFromParcel(Parcel in) {
                return new CustomBottomSheetBehavior3.SavedState(in, null);
            }

            @Override
            public CustomBottomSheetBehavior3.SavedState[] newArray(int size) {
                return new CustomBottomSheetBehavior3.SavedState[size];
            }
        };
    }

    /**
     * A utility function to get the {@link CustomBottomSheetBehavior3} associated with the {@code view}.
     *
     * @param view The {@link View} with {@link CustomBottomSheetBehavior3}.
     * @return The {@link CustomBottomSheetBehavior3} associated with the {@code view}.
     */
    @SuppressWarnings("unchecked")
    public static <V extends View> CustomBottomSheetBehavior3<V> from(V view) {
        ...
        CoordinatorLayout.Behavior behavior = ((CoordinatorLayout.LayoutParams) params)
                .getBehavior();
        if (!(behavior instanceof CustomBottomSheetBehavior3)) {
            throw new IllegalArgumentException(
                    "The view is not associated with CustomBottomSheetBehavior3");
        }
        return (CustomBottomSheetBehavior3<V>) behavior;
    }
}
